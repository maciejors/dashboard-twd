...
def most_listened_to_artist_image(df: pd.DataFrame, size: int = 2):
    """
    ...
    :return: string containing artist's name and image URL separated by
    "|" character
    """
    size = 2 - size  # correction -
    # without it, 0 would mean the biggest image, which is counter-intuitive
    sp = get_sp()
    artist_name = df[["artistName", "trackName"]] \
        .groupby("artistName") \
        .agg("count") \
